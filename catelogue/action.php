...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#' style='background-color:#333'><h4>Categories</h4></a></li>
	";
...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#' style='background-color:#333'><h4>Brands</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
...
if(isset($_POST["page"])){
	...
	for($i=1; $i<=$pageno; $i++){
		echo "
			<li class='active'><a href='#' page='$i' id='page'>$i</a></li>
		";
	}
}
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'><h4>₦ ".number_format($pro_price,0)."
								    <button pid='$pro_id' style='float:right; font-size: 12px' id='product' class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-shopping-cart'></span>AddToCart</button>
									<button style='float:right; font-size: 12px' data-toggle='modal' data-target='#details-1' id='$pro_id' class='btn btn-success btn-xs' onclick='showDetails(this);'>View Detail</button></h4>
								</div>
							</div>
						</div>	
			";
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		$id = $_POST["cat_id"];
		$sql = "SELECT * FROM products WHERE product_cat = '$id' ORDER BY RAND()";
	}else if(isset($_POST["selectBrand"])){
		$id = $_POST["brand_id"];
		$sql = "SELECT * FROM products WHERE product_brand = '$id' ORDER BY RAND()";
	}else {
			...
			echo "
				<div class='col-md-6'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'><h4>₦ ".number_format($pro_price,0)."
								<button pid='$pro_id' style='float:right; font-size: 15px' id='product' class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-shopping-cart'></span>AddToCart</button>
								<button style='float:right; font-size: 15px' data-toggle='modal' data-target='#details-1' id='$pro_id' class='btn btn-success btn-xs' onclick='showDetails(this);'>View Detail</button></h4>
								</div>
							</div>
						</div>	
			";
	...
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			?>
				...
				<a style="float:right;" href="cart.php" class="btn btn-success">Order<span class="glyphicon glyphicon-edit"></span></a>
			<?php
			exit();
		}
	}
						...
						'<div class="row">
								<div class="col-md-2">
									<div class="btn-group">
										...
										<a href="#" update_id="'.$product_id.'" class="btn btn-success update">Update</span></a>
									</div>
								</div>
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/project1/payment_success.php"/>
									...
									<input style="float:right;margin-right:80px;" type="image" name="submit"
										src="http://localhost/GiggleCherries/product_images/logo_order.jpg" alt="Checkout"
